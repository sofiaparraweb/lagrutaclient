...
import { getAllActivity, restoreActivity, deleteActivity } from "../../../Redux/actions.jsx";
...
export default function TablaNoticias() {
  ...
  const handleRestoreClick = (id) => {
    // Llama a la función restoreActivity con el ID de la actividad
    dispatch(restoreActivity(id));
  };

  const handleDeleteClick = (id) => {
    // Llama a la función deleteActivity con el ID de la actividad
  ...
  return (
    <div>
      ...
      <div className="mb-8">
      {Array.isArray(allActivity) && allActivity.length > 0 ? (
          allActivity.map((n) => (
          <div className="bg-secondary-100 p-8 rounded-xl" key={n.id}>
                ...
                >
                  <MenuItem className="p-0 hover:bg-transparent">
                    <button
                      ...
                      onClick={() => handleRestoreClick(n.id)} // Llama a handleRestoreClick con el ID
                    >
                      Restaurar
                    </button>
                    <button
                      className="rounded-lg transition-colors text-gray-700 hover:bg-secondary-900 flex items-center gap-x-4 p-2 flex-1"
                      onClick={() => handleDeleteClick(n.id)} // Llama a handleDeleteClick con el ID
                    >
                      Eliminar
                    </button>
                  </MenuItem>
